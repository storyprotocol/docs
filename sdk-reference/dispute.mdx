...
- `request.targetTag`: The target tag of the dispute. See [dispute tags](/developers/dispute-module#dispute-tags). **Example: "IMPROPER_REGISTRATION"**
...
const response = await client.dispute.raiseDispute({
  ...
  // https://docs.story.foundation/concepts/dispute-module#dispute-tags
...
```typescript Request Type
export type DisputeAssertionRequest = WithTxOptions & {
  ipId: Address;
  assertionId: number | string | bigint;
...
```typescript TypeScript
const response = await client.dispute.disputeIdToAssertionId(1);
```
