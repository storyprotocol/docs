...
> Associated Docs: [royalty.payRoyaltyOnBehalf](/sdk-reference/royalty#payroyaltyonbehalf)
...
Let's say the child earned 1,000 USD off-chain, and is linked to a huge ancestor tree where each parent has a different set of complex license terms. In this scenario, you won't be able to individually calculate each payment to each parent. Instead, you would just pay _yourself_ the amount you earned, and the [Royalty Module](/developers/royalty-module) will automate the payment, such that each ancestor gets their due share.
