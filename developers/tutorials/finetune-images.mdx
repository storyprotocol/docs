...
description: Learn how to use the FLUX Finetuning API to finetune images and then register the output on Story in TypeScript.
---
...
Next we will register this image on Story as an [IP Asset](/developers/ip-asset) in order to monetize and license the IP. Create a `story` folder and add a `utils.ts` file. In there, add the following code to set up your Story Config:
...
View the [IPA Metadata Standard](/developers/ipa-metadata-standard) and construct the metadata for your IP as shown below:
