...
<CardGroup cols={1}>
  <Card
    ...
    href="/concepts/ip-asset#nft-vs-ip-metadata"
    icon="home"
    iconType="solid"
  >
...
All this is doing is calling the `register` function on the [IP Asset Registry](/concepts/ip-asset-registry), which creates a new [IP Asset](/concepts/ip-asset) in our protocol, and returns an `ipId`.

## 4. Set Metadata on IP

Now that we have registered a new IP Asset, we can take our metadata from before and set it on the NFT & IP with the `CoreMetadataModule.sol`. As described [here](/concepts/ip-asset#adding-nft--ip-metadata-to-ip-asset), we need to set 4 params:
