...
[PiPi](https://pfp3.io/pipi/mint) is a free generative pfp project on Story that lets you mint derivative artworks of [Ippy](https://explorer.story.foundation/ipa/0xB1D831271A68Db5c18c8F0B69327446f7C8D0A42), Story's official mascot. Ippy has [Non-Commercial Social Remixing (NCSR)](/developers/pil-flavors#flavor-1-non-commercial-social-remixing) terms attached, which means anyone can use it or create derivative works as long as it's not used commercially and proper attribution is shown.
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    href="https://www.storyscan.xyz/address/0x5C6b236A100d09f8A625dB87E11122749A9B71A6?tab=contract"
    icon="scroll"
    iconType="solid"
    target="_blank"
  >
...
<img
  src="https://files.readme.io/0d7c3ca3a88a9906dac899d6a776417f1c01a07cfa01d9f732974260fd9ea469-Screenshot_2025-02-18_at_10.03.13_AM.png"
...
<CardGroup cols={1}>
  <Card
    ...
    href="/developers/ip-asset#nft-vs-ip-metadata"
    icon="home"
    iconType="solid"
    target="_blank"
  >
...
When you want to register an IP on Story, you must first mint an NFT. This NFT represents the **ownership** over the [IP Asset](/developers/ip-asset).
...
All this is doing is calling the `register` function on the [IP Asset Registry](/developers/ip-asset-registry), which creates a new [IP Asset](/developers/ip-asset) in our protocol, and returns an `ipId`.

## 4. Set Metadata on IP

Now that we have registered a new IP Asset, we can take our metadata from before and set it on the NFT & IP with the `CoreMetadataModule.sol`. As described [here](/developers/ip-asset#adding-nft--ip-metadata-to-ip-asset), we need to set 4 params:
...
This function calls `registerDerivative` in the [Licensing Module](/developers/licensing-module), with:
...
* `licenseTermsIds`: an array containing `1`, which is the license term ID of [Non-Commercial Social Remixing (NCSR)](/developers/pil-flavors#flavor-1-non-commercial-social-remixing). This means the derivative can use Ippy for free but not commercialize it
