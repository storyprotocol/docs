...
View the [IPA Metadata Standard](/developers/ipa-metadata-standard) and construct the metadata for your IP. Properly format your metadata as shown below:
...
When registering your image on Story, you can attach [License Terms](/developers/license-terms) to the IP. These are real, legally binding terms enforced on-chain by the [Licensing Module](/developers/licensing-module), disputable by the [Dispute Module](/developers/dispute-module), and in the worst case, able to be enforced off-chain in court through traditional means.
...
Next we will mint an NFT, register it as an [IP Asset](/developers/ip-asset), set [License Terms](/developers/license-terms) on the IP, and then set both NFT & IP metadata.
...
<CardGroup cols={1}>
  <Card
    ...
    iconType="solid"
  >
    Explore more tutorials in our TypeScript SDK documentation
  </Card>
</CardGroup>
