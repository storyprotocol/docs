...
A piece of Intellectual Property is represented as an [🧩 IP Asset](/concepts/ip-asset) and its associated [⚙️ IP Account](/concepts/ip-account), a smart contract designed to serve as the core identity for each IP. We also have various [🧱 Modules](/concepts/story-modules) to add functionality to IP Assets, like creating derivatives of them, disputing IP, and automating revenue flow between them.

# Architecture

<img src="https://files.readme.io/251dabc-story-v1-architecture.png" alt="Story Protocol Architecture" className="mx-auto" />
...
Then, you **register** the NFT in our protocol through the [IP Asset Registry](/concepts/ip-asset-registry). This deploys an [⚙️ IP Account](/concepts/ip-account), effectively creating an "IP Asset". The address of that contract is the identifier for the IP Asset (the `ipId`).
...
## [⚙️ IP Account](/concepts/ip-account)
...
## [🧱 Modules](/concepts/story-modules)
