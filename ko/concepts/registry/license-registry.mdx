...
description: Story의 라이선스 관련 상태를 관리하는 레지스트리에 대해 알아보세요.
---
...
<CardGroup cols={1}>
  <Card title="LicenseRegistry.sol" href="https://github.com/storyprotocol/protocol-core-v1/blob/main/contracts/registries/LicenseRegistry.sol" icon="scroll" color="#ccb092">
    License Registry의 스마트 컨트랙트를 확인하세요.
  </Card>
</CardGroup>

License Registry는 프로토콜 내의 모든 라이선스 관련 상태를 저장하며, 새로운 License Template 등록과 같은 전역 상태를 관리합니다. 예를 들어 [Programmable IP License (PIL💊)](/concepts/programmable-ip-license/overview), 개별 [IP Assets](/concepts/ip-asset/overview)에 라이선스 첨부, 파생작 등록 등이 있습니다:
...
이 함수를 사용하면 IP Asset을 다른 IP Asset의 파생작으로 등록할 수 있으며, [💸 Royalty Module](/concepts/royalty-module/overview)에서 청구 가능한 로열티 흐름과 같은 기능을 활성화할 수 있습니다.
