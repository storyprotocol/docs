...
description: IP 자산의 수익을 관리하는 IP 로열티 볼트에 대해 알아보기
---

<Accordion title="Skip the Read - 1 Minute Summary" icon="circle-info">
  IP 로열티 볼트는 IP 자산과 관련된 모든 금전적 유입을 위한 풀입니다.
...
각 IP 자산에는 IP 로열티 볼트가 있으며, 이는 IP 자산의 상업적 탐색이나 라이선스 발행으로부터 발생하는 모든 금전적 유입을 위한 풀 역할을 합니다. 로열티 토큰(아래 정의)을 보유한 사람은 누구나 이 풀에서 자신의 몫을 청구할 권리가 있습니다.
...
<CardGroup cols={1}>
  <Card title="IPRoyaltyVault.sol" href="https://github.com/storyprotocol/protocol-core-v1/blob/main/contracts/modules/royalty/policies/IpRoyaltyVault.sol" icon="scroll" color="#ccb092">
    IP 로열티 볼트의 스마트 컨트랙트를 확인하세요.
  </Card>
</CardGroup>

## 토큰 용어

1. **로열티 토큰**: IP 로열티 볼트 컨트랙트는 각 IP 자산의 로열티 토큰을 위한 ERC-20 컨트랙트이기도 합니다. 이는 IP 자산의 IP 로열티 볼트 주소가 로열티 토큰의 ERC-20 토큰 주소이기도 하다는 것을 의미합니다. 각 IP 자산에는 100,000,000개의 로열티 토큰이 연결되어 있으며, 각 토큰은 해당 수익의 0.000001%를 나타냅니다. 이러한 로열티 토큰의 보유자는 관련 IP 로열티 볼트에 있는 Revenue Tokens(아래 정의)를 청구할 수 있습니다.
...
## 로열티 토큰은 어떻게 얻을 수 있나요?

IP 자산이 수익을 받으면 해당 IP 로열티 볼트에 예치됩니다. 이 볼트에서 수익을 청구하려면 관련 로열티 토큰을 보유해야 합니다. 주소가 특정 IP 자산의 로열티 토큰을 보유하게 되면, 해당 주소는 IP 로열티 볼트에서 받은 모든 미래의 Revenue Token(화이트리스트에 등록된)의 해당 비율(보유한 로열티 토큰의 총 공급량 대비 %)을 받을 자격이 있습니다.

IP 로열티 볼트 배포를 트리거하고 초기 로열티 토큰 분배를 하는 두 가지 방법이 있습니다 - 둘 중 먼저 발생하는 것:
...
1. Revenue Tokens는 로열티 모듈 컨트랙트로 흐릅니다. 이 컨트랙트는 수신 IPA의 **royalty stack**에 따라 토큰을 분할합니다. 이 경우, IPA3는 15%의 로열티 스택을 가지고 있어서, 850k 토큰이 IP 로열티 볼트 3으로 흐르고, 150k 토큰이 LAP 컨트랙트로 흐릅니다.
...
3. 이제 Revenue Tokens가 IP 로열티 볼트에 있으므로, 관련 로열티 토큰 보유자들이 볼트에서 청구할 수 있습니다. Revenue Tokens는 로열티 토큰을 보유한 사람에게 청구된다는 것을 기억하세요. 가장 일반적인 경우, 로열티 토큰은 원래 발생한 IP 계정에 있습니다. 청구하려면 다음 중 하나를 호출해야 합니다 `claimRevenueOnBehalfByTokenBatch` 또는 `claimRevenueOnBehalf`.
...
수익 토큰은 다음 함수들을 통해 한 금고에서 다른 금고로 이동할 수 있습니다 `claimByTokenBatchAsSelf` 위치한 `IpRoyaltyVault.sol` 계약에서. 이를 위해서는 수익 토큰을 청구하는 금고가 청구 대상 금고의 로열티 토큰을 소유하고 있어야 합니다. 이는 외부 로열티 정책과 함께 사용할 때 특히 유용할 수 있습니다.
