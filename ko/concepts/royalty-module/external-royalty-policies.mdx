...
IPA 소유자는 IP가 리믹스될 때 허용하고자 하는 로열티 정책을 결정합니다. LAP, LRP 및 기타 External Royalty Policy와 같은 다양한 로열티 규칙 옵션 중에서 선택할 수 있습니다. 사용자가 "Policy X"로 라이선스 토큰을 발행하기로 결정했다고 가정해보겠습니다. 그 후, IP2가 IP1을 리믹스하고 IP3가 IP2를 리믹스하여 아래 이미지와 같은 상황이 됩니다:
...
* "Policy X"는 RT2 토큰 공급량의 3%를 받아 사용자 기반에 재분배할 수 있습니다. IP1 소유자는 10%를 원했지만 - 예시를 위해 가정해보면 - "Policy X"의 특정 사용 사례와 맞춤 로직으로 인해, IP2 소유자는 IP를 홍보할 수 있는 매우 큰 유통 네트워크를 보유하고 있어 플랫폼에서 특별한 지위를 부여받아 부모 IP에게 제공해야 하는 지분 비율에서 70% 할인을 받습니다. 따라서 라이선스 비율이 지정한 10%를 제공하는 대신 3%만 제공합니다.
...
* RT3 토큰 공급량의 50%를 보유한 IP Account 3이 50만 WIP를 청구
* RT3 토큰 공급량의 10%를 보유한 IP1 로열티 금고가 `claimByTokenBatchAsSelf` 함수를 통해 10만 WIP를 청구
* RT3 토큰 공급량의 40%를 보유한 "Policy X"가 40만 WIP를 청구합니다. 이 금액은 "Policy X" 맞춤 계약에 따라 특정 규칙에 따라 - y%와 z%를 정의하여 - 사용자들에게 추가로 분배됩니다.
