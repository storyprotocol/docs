...
이전까지는 여러 IP를 등록하거나 민팅, 라이선스 조건 첨부, 파생물 등록과 같은 다른 작업을 수행하려면 각 작업마다 별도의 트랜잭션이 필요했습니다. 이는 비효율적이고 비용이 많이 들 수 있습니다. 이 프로세스를 간소화하기 위해 여러 트랜잭션을 하나로 일괄 처리할 수 있습니다. 이를 위한 두 가지 솔루션이 있습니다:
...
이를 위해 SPG의 `multicall` 함수를 사용하여 `mintAndRegisterIpAndMakeDerivative` 함수 호출을 일괄 처리할 수 있습니다.
