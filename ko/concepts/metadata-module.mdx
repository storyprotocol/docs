...
`CoreMetadataModule.sol`는 IP 자산의 메타데이터 쓰기 및 업데이트를 담당합니다. 이는 **stateful**하며 다음과 같은 주요 기능을 제공합니다:
...
메타데이터 모듈은 IP 자산 메타데이터를 관리하기 위한 강력한 시스템을 제공하여 지적 재산에 대한 중요한 정보가 적절하게 기록되고, 접근 가능하며, 필요할 때 불변성을 가질 수 있도록 보장합니다.
