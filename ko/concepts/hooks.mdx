...
가장 중요한 것은, Licensing Hook이 다음을 구현한다는 것입니다 `beforeMintLicenseTokens` 함수로, 이는 License Token이 발행되기 전에 호출되어 다음을 구현합니다 [사용자 정의 로직](/concepts/licensing-module/license-config#logic-that-is-possible-with-license-config) 그리고 최종 결정 `totalMintingFee` 해당 License Token의.
...
| 발행 수수료                                                | 중요도    |
| ----------------------------------------------------- | ------ |
| The `totalMintingFee`에서 반환된 `beforeMintLicenseTokens` | 최우선 순위 |
| The `mintingFee`에 설정된 `LicenseConfig`                 | ⬇️     |
| The `mintingFee`라이선스 조건에 설정된                          | 최하위 순위 |

<Warning>
  외부 라이선스 hooks의 잠재적으로 악의적인 구현을 주의하세요.
  선택한 hook의 코드를 먼저 확인하시기 바랍니다. Story 팀이
  검토하거나 감사하지 않았을 수 있습니다.
</Warning>
...
<Info>
  이러한 hooks의 배포된 주소를 확인하세요 [here](/developers/deployed-smart-contracts#license-hooks).
</Info>
...
Licensing Hooks는 궁극적으로 다음 인터페이스를 구현하는 스마트 계약입니다 `ILicensingHook`. 인터페이스는 다음에서 확인할 수 있습니다 [here](https://github.com/storyprotocol/protocol-periphery-v1/blob/main/contracts/interfaces/ILicensingHook.sol). 우리는 이미 몇 가지 Licensing Hooks를 배포했습니다 (위의 차트를 참조하세요).
...
<Steps>
  ...
  <Step title="Set the Licensing Config">
    다음으로, License Terms에 Licensing Config를 설정하겠습니다. 다음 예제에서는 IP Asset을 등록할 때 이 작업이 이루어지는 것을 보여줍니다:
    ...
    <Note>
      이는 다음을 사용합니다 `mintAndRegisterIpAssetWithPilTerms` 메서드는 다음에서 찾을 수 있습니다 [here](/sdk-reference/ipasset#mintandregisteripassetwithpilterms).
    </Note>
  ...
  <Step title="Set the Limit to 1">
    이제 우리의 조건에 Licensing Config를 설정했으므로, hook의 `setTotalLicenseTokenLimit` 함수를 호출하고 발행할 수 있는 최대 라이선스 수를 1로 설정할 수 있습니다.
...
<Warning>
  문서는 곧 제공될 예정입니다. 그동안 질문이 있으시다면 다음에서 문의하세요 [Builder's Discord](https://discord.gg/storybuilders).
</Warning>
