...
PILicenseTemplate(프로그래밍 가능한 IP 라이선스 템플릿)은 Story에서 IP 자산에 대한 라이선스 조항을 정의하고 관리하는 스마트 계약입니다. IP 소유자가 자신의 IP 자산에 첨부할 수 있는 맞춤형 라이선스 조항을 만들 수 있게 하여, IP의 상업적 사용과 파생 작품 제작을 제어할 수 있게 합니다.
...
라이선스 조항과 토큰을 추적하는 라이선스 레지스트리 계약의 주소입니다.
...
라이선스 조항 ID를 해당하는 PILTerms 구조체에 매핑합니다.
...
라이선스 조항의 해시를 해당하는 라이선스 조항 ID에 매핑합니다.
...
새로운 라이선스 조항을 등록하고 새로 등록된 라이선스 조항의 ID를 반환합니다. 라이선스 조항은 해시되며 이 해시를 사용하여 조항이 이미 등록되어 있는지 확인합니다. 조항이 이미 등록되어 있다면 기존 ID를 반환합니다.
...
라이선스 조항이 존재하는지 확인합니다.

**Parameters:**

* `licenseTermsId`: 라이선스 조항의 ID입니다.

**Returns:**

* 라이선스 조항이 존재하면 true를, 그렇지 않으면 false를 반환합니다.
...
라이선스 토큰의 발행을 검증합니다. 이 함수는 LicensingModule이 라이선스 토큰을 발행할 때 호출되어 라이선스 조항에 따라 발행이 허용되는지 확인합니다.

**Parameters:**

* `licenseTermsId`: 라이선스 조항의 ID입니다.
* `licensee`: 라이선스 토큰을 받을 라이선스 사용자의 주소입니다.
* `licensorIpId`: 라이선스 토큰을 발행하는 라이선스 조항을 첨부한 라이선스 제공자의 IP ID입니다.

**Returns:**

* 발행이 검증되면 true를, 그렇지 않으면 false를 반환합니다.
...
파생 작품의 등록을 검증합니다. 이 함수는 LicensingModule이 파생 작품을 등록하는 동안 호출되어 원본 IP의 라이선스 조항을 준수하는지 확인합니다.

**Parameters:**

* `childIpId`: 파생 작품의 IP ID입니다.
* `parentIpId`: 원본 작품의 IP ID입니다.
* `licenseTermsId`: 라이선스 조항의 ID입니다.
* `licensee`: 라이선스 사용자의 주소입니다.

**Returns:**

* 등록이 검증되면 true를, 그렇지 않으면 false를 반환합니다.
...
라이선스들이 서로 호환되는지 검증합니다. 이 함수는 LicensingModule이 여러 원본 IP에 대한 파생 IP를 등록할 때 라이선스 호환성을 확인하기 위해 호출됩니다.

**Parameters:**

* `licenseTermsIds`: 라이선스 조항들의 ID입니다.
...
모든 부모 IP에 대한 파생물 등록을 검증합니다. 이 함수는 여러 부모 IP에 대한 파생 IP 등록을 위한 라이선스를 검증하기 위해 LicensingModule에 의해 호출됩니다.

**Parameters:**

* `childIpId`: 파생물의 IP ID.
* `parentIpIds`: 부모들의 IP ID.
* `licenseTermsIds`: 라이선스 조건들의 ID.
* `childIpOwner`: 파생 IP 소유자의 주소.
...
* `royaltyPolicy`: 라이선스 조건에 지정된 로열티 정책의 주소.
* `royaltyData`: 로열티 정책의 데이터.
* `mintingFee`: 라이선스 발행 수수료.
* `currency`: 라이선스 발행 수수료와 로열티에 사용되는 ERC20 토큰의 주소.
...
* 라이선스 조건이 양도 가능하면 true를, 그렇지 않으면 false를 반환합니다.
...
* 컨트랙트가 인터페이스를 지원하면 true를, 그렇지 않으면 false를 반환합니다.
...
* `royaltyPolicy`: StoryProtocol에서 사전에 요구하는 로열티 정책 컨트랙트의 주소.
* `mintingFee`: 라이선스 발행 시 지불해야 하는 수수료.
* `expiration`: 라이선스의 만료 기간.
* `commercialUse`: 작품을 상업적으로 사용할 수 있는지 여부를 나타냅니다.
* `commercialAttribution`: 작품을 상업적으로 복제할 때 저작자 표시가 필요한지 여부.
* `commercializerChecker`: 작품을 상업적으로 이용할 수 있는 상업화 주체. 제로 주소인 경우 제한이 없습니다.
* `commercializerCheckerData`: 상업화 검사기 계약에 전달될 데이터입니다.
* `commercialRevShare`: 라이센서와 공유해야 하는 수익의 비율입니다.
* `commercialRevCelling`: 작품의 상업적 사용으로 발생할 수 있는 최대 수익입니다.
* `derivativesAllowed`: 라이센시가 자신의 작품의 파생물을 만들 수 있는지 여부를 나타냅니다.
* `derivativesAttribution`: 작품의 파생물에 대해 저작자 표시가 필요한지 여부를 나타냅니다.
* `derivativesApproval`: 작품의 파생물이 라이센서 IP ID에 연결되기 전에 라이센서의 승인이 필요한지 여부를 나타냅니다.
* `derivativesReciprocal`: 라이센시가 작품의 파생물을 동일한 조건으로 라이센스해야 하는지 여부를 나타냅니다.
* `derivativeRevCelling`: 작품의 파생적 사용으로 발생할 수 있는 최대 수익입니다.
* `currency`: 민팅 수수료를 지불하는 데 사용될 ERC20 토큰입니다. 토큰은 Story Protocol에 등록되어 있어야 합니다.
* `uri`: 오프체인 라이센스 조건을 가져오는 데 사용할 수 있는 라이센스 조건의 URI입니다.
