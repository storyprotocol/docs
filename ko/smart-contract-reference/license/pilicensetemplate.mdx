...
새로운 라이선스 조항을 등록하고 새로 등록된 라이선스 조항의 ID를 반환합니다. 라이선스 조항은 해시화되며 이 해시를 사용하여 조항이 이미 등록되어 있는지 확인합니다. 조항이 이미 등록되어 있다면 기존 ID를 반환합니다.
...
파생 작품의 등록을 검증합니다. 이 함수는 파생 작품 등록 시 LicensingModule에 의해 호출되어 원본 IP의 라이선스 조항을 준수하는지 확인합니다.
...
라이선스들의 호환성을 검증합니다. 이 함수는 여러 원본 IP에 대한 파생 IP를 등록할 때 LicensingModule에 의해 호출되어 라이선스 호환성을 확인합니다.
...
* `derivativesApproval`: 라이센서 IP ID에 연결되기 전에 라이센서가 작품의 파생물을 승인해야 하는지 여부를 나타냅니다.
