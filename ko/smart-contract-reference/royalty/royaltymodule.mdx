...
RoyaltyModule은 Story에서 로열티 지불을 처리하는 주요 진입점입니다. IP 소유자가 IP 자산에 대한 로열티 정책을 설정하고 파생 IP 소유자가 상위 IP에 로열티를 지불할 수 있도록 합니다.
...
4. **일시 중지 기능**: 컨트랙트는 비상 상황에서 `whenNotPaused` 수정자를 사용하여 일시 중지될 수 있습니다.
