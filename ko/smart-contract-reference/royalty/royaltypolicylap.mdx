...
* `transferredTokenLAP`: LAP를 통해 하위 IP로부터 금고로 이전된 총 생애 수익 토큰
...
1. **접근 제어**: 다음과 같은 함수들 `onLicenseMinting` 및 `onLinkToParents` 는 `onlyRoyaltyModule` 수정자를 통해 Royalty Module에서만 호출할 수 있습니다.
...
6. **입력 유효성 검사**: 계약은 입력값을 검증하고 동일한 IP 간의 전송을 방지하는 등의 엣지 케이스를 확인합니다.
