...
RoyaltyPolicyLAP(유동적 절대 비율) 계약은 유동적 절대 비율 메커니즘을 사용하여 주어진 IP 자산에 대한 로열티 분배 로직을 정의합니다. IP 자산과 그 조상들 간의 로열티 관계를 관리하여 적절한 로열티 금고로 수익 토큰을 이전할 수 있게 합니다.
...
* `royaltyStackLAP`: LAP 로열티 정책에 대해 모든 조상들에게 지불해야 할 로열티 비율의 합
* `ancestorPercentLAP`: LAP 로열티 정책에 대한 IP 자산과 주어진 조상 간의 로열티 비율
* `transferredTokenLAP`: LAP를 통해 자손 IP로부터 금고로 이전된 총 생애 수익 토큰
...
라이센스 민팅 시 로열티 관련 로직을 실행합니다.

**Parameters:**

* `ipId`: 라이센스가 민팅되는 ipId(라이센서)
* `licensePercent`: 민팅되는 라이센스의 라이센스 비율
...
부모에 연결할 때 로열티 관련 로직을 실행합니다.

**Parameters:**

* `ipId`: 부모에 연결되는 자식 ipId
* `parentIpIds`: 자식 ipId가 연결되는 부모 ipId들
* `licenseRoyaltyPolicies`: 라이센스의 로열티 정책들
* `licensesPercent`: 민팅되는 라이센스의 라이센스 비율
...
LAP 로열티 정책을 통해 청구 가능한 수익 토큰의 금액을 금고로 이전합니다.
...
* `ipId`: IP 자산의 ipId
* `licensePercent`: 라이센스의 비율

**Returns:**

* 자식을 주어진 IP 자산에 연결하는 데 필요한 로열티 토큰의 양(LAP의 경우 항상 0)
...
* LAP 로열티 정책에 대해 모든 조상에게 지급될 로열티 비율의 합
...
LAP를 통한 IP 자산과 그 조상들 간의 로열티 비율을 반환합니다.
...
* LAP를 통한 IP 자산과 그 조상들 간의 로열티 비율
...
LAP를 통해 하위 IP로부터 볼트로 이전된 총 평생 수익 토큰을 반환합니다.
...
* LAP를 통해 하위 IP로부터 볼트로 이전된 총 평생 수익 토큰
...
* False (LAP 로열티 정책은 그룹 작업을 지원하지 않음)
...
RoyaltyPolicyLAP의 스토리지 구조체를 반환합니다.

**Returns:**

* RoyaltyPolicyLAP의 스토리지 구조
...
RoyaltyPolicyLAP 컨트랙트는 다음과 같은 여러 보안 조치를 구현합니다:

1. **접근 제어**: 다음과 같은 함수들 `onLicenseMinting` 및 `onLinkToParents` 는 오직 `onlyRoyaltyModule` 수정자를 통해 Royalty Module에 의해서만 호출될 수 있습니다.
...
6. **입력값 검증**: 계약은 입력값을 검증하고 동일한 IP 간의 전송을 방지하는 등의 엣지 케이스를 확인합니다.
