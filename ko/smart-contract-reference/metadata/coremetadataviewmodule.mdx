...
CoreMetadataViewModule은 Story 내의 IP 자산의 핵심 메타데이터에 대한 읽기 전용 접근을 제공하는 뷰 모듈입니다. IP 자산에서 metadataURI, metadataHash, NFT 토큰 URI, 등록 날짜와 같은 메타데이터 정보를 검색합니다.
...
ModuleRegistry에서 검색하여 이 뷰 모듈이 사용하는 CoreMetadataModule의 주소를 업데이트합니다.
...
IP 자산이 연결된 NFT의 TokenURI를 검색하며, 가능한 경우 CoreMetadataModule의 TokenURI를 우선적으로 사용합니다.
...
* `string`: IP 자산에 연결된 NFT TokenURI입니다.
...
CoreMetadataModule에서 설정한 IP 자산의 NFT 메타데이터 해시를 검색합니다.
...
IP 자산에 대한 표준 NFT 메타데이터 스키마에 따라 형식화된 JSON 문자열을 생성하며, 모든 관련 메타데이터 필드를 포함합니다. 이 함수는 IPAssetRegistry와 CoreMetadataModule 모두에서 메타데이터를 통합하며, CoreMetadataModule의 "NFT TokenURI"가 우선순위를 가집니다.
...
* `string`: IP 자산의 모든 메타데이터를 나타내는 base64로 인코딩된 JSON 문자열.
...
* `bool`: 뷰 모듈이 지원되면 true, 그렇지 않으면 false.
