...
IP 자산에 대한 표준 NFT 메타데이터 스키마에 따라 형식이 지정된 JSON 문자열을 생성하며, 모든 관련 메타데이터 필드를 포함합니다. 이 함수는 IPAssetRegistry와 CoreMetadataModule의 메타데이터를 통합하며, CoreMetadataModule의 "NFT TokenURI"가 우선순위를 가집니다.
