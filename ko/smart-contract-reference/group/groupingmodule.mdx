...
Story의 IPA 그룹화를 위한 메인 진입점인 Grouping Module은 다음을 담당합니다:
...
* `allowed`: 그룹 보상 풀이 화이트리스트에 등록되어 있는지 여부입니다.
...
그룹에 IP를 추가합니다. 이 함수는 Group IP 소유자나 승인된 운영자가 호출해야 합니다.
...
그룹에서 IP를 제거합니다. 이 함수는 Group IP 소유자나 승인된 운영자가 호출해야 합니다.
