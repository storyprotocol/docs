...
* `groupLicenseTermsId`: 그룹에 연결된 라이선스 조항의 ID.
...
* `licenseTemplate`: 라이선스 조항이 정의된 라이선스 템플릿의 주소.
* `licenseTermsId`: 라이선스 조항의 ID.

**Returns:**

* `bool`: 라이선스 조항이 존재하는지 여부.
...
인덱스로 IP의 연결된 라이선스 조항을 가져옵니다.

**Parameters:**

* `ipId`: IP의 주소.
* `index`: IP의 모든 연결된 라이선스 조항 배열 내의 연결된 라이선스 조항의 인덱스.
