...
IPAccountRegistry는 IP 계정의 등록과 추적을 관리하는 책임이 있습니다. Story 생태계 내에서 토큰화된 지적 재산권 자산을 나타내는 IPAccount 계약을 배포하기 위해 공개 ERC6551 레지스트리를 활용합니다.
...
* `chainId`: IP 계정이 위치한 체인 ID입니다.
* `tokenContract`: IP 계정과 연결된 토큰 계약의 주소입니다.
* `tokenId`: IP 계정과 연결된 토큰의 ID입니다.

**Returns:**

* `ipAccountAddress`: 주어진 NFT 토큰과 연결된 IP 계정의 주소입니다.
...
* `chainId`: IP 계정이 생성될 체인 ID입니다.
* `tokenContract`: IP 계정과 연결될 토큰 계약의 주소입니다.
* `tokenId`: IP 계정과 연결될 토큰의 ID입니다.

**Returns:**

* `ipAccountAddress`: 새로 생성된 IP 계정의 주소입니다.
