...
라이선스 토큰 없이 부모 IP의 라이선스 조건으로 직접 파생작을 등록하고, 부모 IP의 라이선스 조건을 파생 IP에 연결합니다.
...
* `max_revenue_share`: \[선택사항] 자식이 파생작으로 등록할 때 자식과 부모 간에 합의된 최대 수익 공유 비율. 0에서 100 사이여야 합니다.**Default: 100**
