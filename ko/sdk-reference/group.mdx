...
* `request.groupPool`: 그룹 내 IP 풀 간의 로열티 분배 방식을 지정하는 주소.
* `request.maxAllowedRevShare`: 각 멤버 IP에 할당될 수 있는 최대 보상 공유 비율.
* `request.licenseData`: 새 그룹 IP에 첨부될 라이선스와 그 구성 데이터.
  * `request.licenseData.licenseTermsId`: 새 그룹 IP에 첨부될 등록된 라이선스 약관의 ID입니다.
  * `request.licenseData.licensingConfig`: \[선택사항] 다음을 참조하세요 [LicensingConfig 타입](https://github.com/storyprotocol/sdk/blob/main/packages/core-sdk/src/types/common.ts#L15). 제공되지 않은 경우, [여기](https://github.com/storyprotocol/sdk/blob/main/packages/core-sdk/src/utils/validateLicenseConfig.ts)에 표시된 기본값이 사용됩니다.
  * `request.licenseData.licenseTemplate`: \[선택사항] 새 그룹 IP에 첨부될 라이선스 템플릿의 주소입니다. 기본값은 프로그래머블 IP 라이선스입니다.
