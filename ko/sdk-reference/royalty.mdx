---
title: Royalty
description: RoyaltyClient를 사용하여 Story 내에서 로열티 지급과 청구를 관리할 수 있습니다.
---
...
* `request.wipOptions`: \[선택사항]
  ...
  * `request.wipOptions.enableAutoWrapIp`: \[선택사항] 기본적으로 현재 WIP 잔액이 수수료를 충당하지 못하는 경우 IP가 WIP로 변환됩니다. 이 동작을 비활성화하려면 `false`로 설정하세요.**Default: true**
  * `request.wipOptions.enableAutoApprove`: \[선택사항] WIP가 필요하지만 현재 허용량이 충분하지 않을 때 자동으로 WIP 사용을 승인합니다. 이 동작을 비활성화하려면 `false`로 설정하세요.**Default: true**
* `request.erc20Options`: \[선택사항]
  * `request.erc20Options.enableAutoApprove`: \[선택사항] ERC20가 필요하지만 현재 허용량이 충분하지 않을 때 자동으로 ERC20 사용을 승인합니다. 이 동작을 비활성화하려면 `false`로 설정하세요.**Default: true**
...
* `request.claimer`: 통화(수익) 토큰의 청구자 주소. IP가 모든 로열티 토큰을 보유하고 있는 경우 일반적으로 상위 IP의 ipId입니다. 그렇지 않은 경우, 상위 IP 로열티 토큰을 보유하고 있는 주소가 됩니다.
...
* `request.currencyTokens[]`: 로열티가 청구될 통화 토큰의 주소들.
...
* `request.ancestorIps[]`: 수익이 청구되는 상위 IP 정보의 배열.
  * `request.ancestorIps[].ipId`: 수익이 청구되는 상위 IP의 주소.
  * `request.ancestorIps[].claimer`: 통화(수익) 토큰의 청구자 주소. IP가 모든 로열티 토큰을 보유하고 있는 경우 일반적으로 상위 IP의 ipId입니다. 그렇지 않은 경우, 상위 IP 로열티 토큰을 보유하고 있는 주소가 됩니다.
  ...
  * `request.ancestorIps[].currencyTokens[]`: 로열티가 청구될 통화 토큰의 주소들.
* `request.claimOptions`: \[선택사항]
  * `request.claimOptions.autoTransferAllClaimedTokensFromIp`: \[선택사항] 활성화된 경우, 지갑이 IP를 소유하고 있다면 청구자의 모든 청구된 토큰이 지갑 주소로 이전됩니다. 지갑이 청구자이거나 청구자가 지갑이 소유한 IP가 아닌 경우에는 토큰이 이전되지 않습니다. 청구자로부터의 토큰 자동 이전을 비활성화하려면 false로 설정하세요.**Default: true**
  * `request.claimOptions.autoUnwrapIpTokens`: \[선택사항] 기본적으로 모든 청구된 WIP 토큰은 이전된 후 IP로 다시 변환됩니다. 이 동작을 비활성화하려면 false로 설정하세요.**Default: false**
