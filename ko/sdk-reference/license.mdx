...
License Tokens를 발행하여 License Terms에 기반한 IP Asset 사용 권한을 부여합니다. 라이선스 토큰은 에게 발행됩니다.[라이선스 토큰](/concepts/licensing-module/license-token) IP Asset을 사용할 수 있는 권한을 부여하는 [라이선스 조건](/concepts/licensing-module/license-terms). 라이선스 토큰은 다음으로 발행됩니다 `receiver`.

라이선스 토큰은 License Terms가 IP Asset에 이미 첨부되어 있어 공개적으로 사용 가능한 라이선스인 경우에만 발행될 수 있습니다. 하지만 IP 소유자는 IP Asset에 첨부되지 않은 License Terms로 라이선스 토큰을 발행하여 private license를 만들 수 있습니다.`licenseTermsId` 이미 IP Asset에 연결되어 있어 공개적으로 사용 가능한 라이선스가 됩니다. 하지만 IP 소유자는 [비공개 라이선스](/concepts/licensing-module/license-token#private-licenses)를 발행할 수 있으며 이는 `licenseTermsId` IP Asset에 연결되지 않은 라이선스 토큰을 발행함으로써 가능합니다.

<Warning>
  라이선스 조건이나 IP 소유자의 설정에 따라 발행 수수료가 필요할 수 있습니다. 발행 수수료는 라이선스 조건에 명시되거나 IP 소유자가 설정한 발행 수수료 토큰으로 지불됩니다. IP 소유자는 자신의 IP의 발행 수수료를 설정하거나 발행 수수료 모듈을 설정하여 발행 수수료를 결정할 수 있습니다.
</Warning>
...
* 예상 매개변수: 여기에 모든 예상 매개변수를 나열하는 대신 이 파일의 타입을 참조하세요. 이들은 모두 PIL Terms에서 가져옵니다.`LicenseTerms` 유형은 [이](https://github.com/storyprotocol/sdk/blob/main/packages/core-sdk/src/types/resources/license.ts) 파일에 있습니다. 이들은 모두 [PIL Terms](/concepts/programmable-ip-license/pil-terms)에서 가져온 것입니다.
* `request.txOptions`: \[선택사항] 트랜잭션 [options](https://github.com/storyprotocol/sdk/blob/main/packages/core-sdk/src/types/options.ts).
...
<Warning>
  이 함수를 호출할 필요가 없습니다. 비상업적 소셜 리믹스 조건은 이미 우리 프로토콜에 등록되어 있습니다. 다시 등록할 필요가 없습니다.`licenseTermdId = 1` 우리 프로토콜에 있습니다. 이들을 다시 등록할 필요가 없습니다.
</Warning>
...
* `request.licenseTemplate`: 사용된 라이선스 템플릿의 주소, 지정되지 않은 경우 구성이 모든 라이선스에 적용됩니다.
