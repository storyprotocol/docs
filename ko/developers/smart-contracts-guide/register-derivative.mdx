---
title: 파생물 등록하기
description: Solidity에서 다른 IP 자산의 자식으로 파생물/리믹스 IP 자산을 등록하는 방법을 알아보세요.
---
...
일단[License Token](/concepts/licensing-module/license-token)이 IP 자산에서 발행되면, 해당 토큰(ERC-721 NFT)의 소유자는 토큰을 소각하여 자신의 IP 자산을 License Token과 연관된 IP 자산의 파생물로 등록할 수 있습니다.
...
작동을 확인하고 결과를 검증하기 위해`test/4_IPARemix.t.sol` 아래에 테스트 파일을 만들어보겠습니다:

<Note>
  **컨트랙트 주소**

  Story 컨트랙트의 주소를 미리 입력해두었습니다. 하지만 다음 위치에서도 주소를 확인할 수 있습니다:[배포된 스마트 컨트랙트](/developers/deployed-smart-contracts)
</Note>
...
실행`forge build`. 모든 것이 정상이라면, 명령어가 성공적으로 컴파일될 것입니다.

다음 명령어를 실행하여 테스트를 진행하세요:
...
이제 부모-자식 IP 관계를 설정했으니, 라이선스 조건에 따른 지불과 자동화된 수익 공유를 살펴볼 수 있습니다. 이는 다음 페이지에서 다룰 예정입니다.
