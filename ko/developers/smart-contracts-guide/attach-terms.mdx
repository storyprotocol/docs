---
title: IPA에 조건 첨부하기
description: Solidity에서 IP 자산에 라이선스 조건을 첨부하는 방법을 알아보세요.
---
...
2. 라이선스 약관을 생성하고 `licenseTermsId`. 다음 페이지를 따라 할 수 있습니다 [previous page](/developers/smart-contracts-guide/register-terms).

## 라이선스 약관 연결

이제 약관을 생성하고 관련된 `licenseTermsId`을 가지고 있으므로, 기존 IP Asset에 연결할 수 있습니다.

테스트 파일을 다음 위치에 생성해 보겠습니다 `test/2_AttachTerms.t.sol` 작동을 확인하고 결과를 검증하기 위해:
