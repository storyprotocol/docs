...
1. 이미 **custom** ERC-721 NFT 컨트랙트가 있고 발행할 수 있는 경우
2. 새로운 [SPG (Periphery)](/concepts/spg/overview) NFT 컨트랙트를 생성하여 발행하려는 경우

## 시나리오 #1: 이미 custom ERC-721 NFT 컨트랙트가 있고 발행할 수 있는 경우

이미 NFT를 발행했거나 custom ERC-721 컨트랙트를 사용하여 IP를 등록하려는 경우 이 섹션을 참고하세요.
...
<Note>
  **컨트랙트 주소**
  ...
  Story 컨트랙트의 주소를 미리 입력해두었습니다. 하지만 다음에서도 주소를 확인할 수 있습니다: [배포된 스마트 컨트랙트](/developers/deployed-smart-contracts)
...
## 시나리오 #2: SPG NFT 컨트랙트를 생성하여 발행하려는 경우

자체 custom NFT 컨트랙트가 없다면 이 섹션을 참고하세요.

이를 위해 우리는 [SPG](/concepts/spg/overview)를 사용할 것입니다. 이는 여러 트랜잭션을 하나로 결합할 수 있게 해주는 유틸리티 컨트랙트입니다. 이 경우, SPG의 `mintAndRegisterIp` 함수를 사용하여 NFT 발행과 IP 자산 등록을 하나로 결합할 것입니다.

를 사용하기 위해서는 `mintAndRegisterIp`먼저 새로운 `SPGNFT` 컬렉션을 생성해야 합니다. `createCollection`를 `StoryProtocolGateway` 컨트랙트에서 호출하면 됩니다. 또는 자체 `SPGNFT`를 만들고 싶다면 [ISPGNFT](https://github.com/storyprotocol/protocol-periphery-v1/blob/main/contracts/interfaces/ISPGNFT.sol) 컨트랙트 인터페이스를 구현할 수 있습니다. 아래 예시를 따라 새로운 SPGNFT를 초기화하는 데 사용할 수 있는 매개변수를 확인하세요.

자체 SPGNFT가 있으면 다음을 호출하기만 하면 됩니다 `mintAndRegisterIp` with:
