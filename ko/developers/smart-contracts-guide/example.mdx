---
title: 예시 사용하기
description: 모든 튜토리얼을 실용적인 예시로 결합해봅시다.
---
...
이제 각각의 개별 단계들을 살펴보았으니, 우리만의 스마트 컨트랙트를 작성하고, 배포하고, 검증해보겠습니다.

## IPA 등록, 라이선스 조건 등록, 그리고 IPA에 연결하기

이 첫 번째 섹션에서는 몇 가지 튜토리얼을 하나로 결합할 것입니다. 우리는 다음과 같은 함수를 만들 것입니다 `mintAndRegisterAndCreateTermsAndAttach` 이 함수는 새로운 IP Asset을 발행하고 등록하며, 새로운 라이선스 조건을 등록하고, 이 조건들을 IP Asset에 연결할 수 있게 해줍니다. 또한 `receiver` 필드를 새로운 IP Asset의 소유자로 받아들일 것입니다.

### 전제 조건

* 완료하기 [IP Asset 등록하기](/developers/smart-contracts-guide/register-ip-asset)
* 완료하기 [라이선스 조건 등록하기](/developers/smart-contracts-guide/register-terms)
* 완료하기 [IPA에 조건 연결하기](/developers/smart-contracts-guide/attach-terms)

### 컨트랙트 작성하기

새로운 파일을 다음 위치에 만드세요 `./src/Example.sol` 그리고 다음 내용을 붙여넣으세요:

<Note>
  **컨트랙트 주소**

  생성자에 전달할 컨트랙트 주소를 얻으려면 다음으로 이동하세요 [배포된 스마트 컨트랙트](/developers/deployed-smart-contracts).
</Note>
...
이 다음 섹션에서는 나중의 몇 가지 튜토리얼을 하나로 결합할 것입니다. 우리는 다음과 같은 함수를 만들 것입니다 `mintLicenseTokenAndRegisterDerivative` 이 함수는 잠재적으로 다른 사용자가 자신만의 "자식"(파생) IP Asset을 등록하고, "부모"(루트) IP Asset으로부터 라이선스 토큰을 발행하며, 자식 IPA를 부모 IPA의 파생물로 등록할 수 있게 해줍니다. 이 함수는 다음과 같은 매개변수들을 받습니다:

1. `parentIpId`: `ipId` 부모 IPA의
2. `licenseTermsId`: 라이선스 토큰을 발행하고자 하는 라이선스 조건의 ID
3. `receiver`: 자식 IPA의 소유자

### 전제 조건

* 완료하기 [라이선스 토큰 발행하기](/developers/smart-contracts-guide/mint-license)
* 완료하기 [파생물 등록하기](/developers/smart-contracts-guide/register-derivative)

### 컨트랙트 작성하기

귀하의 `Example.sol` 컨트랙트에 다음 함수를 하단에 추가하세요:
...
다음 위치에 새로운 파일을 만드세요 `test/Example.t.sol` 그리고 다음 내용을 붙여넣으세요:
...
이 `--constructor-args`는 다음에서 가져옵니다 [배포된 스마트 컨트랙트](/developers/deployed-smart-contracts).
...
모든 것이 올바르게 작동했다면, 콘솔에서 다음과 같은 것을 볼 수 있을 것입니다 `Deployed to: 0xfb0923D531C1ca54AB9ee10CB8364b23d0C7F47d`. 해당 주소를 다음에 붙여넣으세요 [익스플로러](https://aeneid.storyscan.io/) 그리고 검증된 컨트랙트를 확인하세요!
