...
이제 각각의 단계들을 살펴보았으니, 우리만의 스마트 컨트랙트를 작성하고, 배포하고, 검증해보겠습니다.
...
새로운 파일을 생성하세요 `./src/Example.sol` 그리고 다음 내용을 붙여넣으세요:
...
이 다음 섹션에서는 나중의 튜토리얼 몇 가지를 하나로 결합할 것입니다. 우리는 다음과 같은 함수를 만들 것입니다 `mintLicenseTokenAndRegisterDerivative` 이 함수는 잠재적으로 다른 사용자가 자신만의 "자식"(파생) IP Asset을 등록하고, "부모"(루트) IP Asset으로부터 라이선스 토큰을 발행하며, 자식 IPA를 부모 IPA의 파생물로 등록할 수 있게 해줍니다. 이 함수는 다음과 같은 매개변수들을 받습니다:
...
다른 새로운 파일을 생성하세요 `test/Example.t.sol` 그리고 다음 내용을 붙여넣으세요:
