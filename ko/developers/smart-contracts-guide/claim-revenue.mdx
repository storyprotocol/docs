...
두 시나리오 모두 아래의 `payRoyaltyOnBehalf` 함수를 사용합니다. 이 경우 [Royalty Module](/concepts/royalty-module/overview)이 자동으로 다양한 지불 흐름을 처리하여 지불되는 IPA와 특정 `commercialRevShare`을 협상한 상위 IP 자산이 자신의 몫을 청구할 수 있도록 합니다.
...
2. 기본적인 이해 [Royalty Module](/concepts/royalty-module/overview)
3. 이 예제가 작동하도록 하기 위해 상업적 수익 공유가 포함된 라이선스 약관이 있는 자식 IPA와 부모 IPA

## 시작하기 전에

IP Asset에 다음을 사용하여 지불할 수 있습니다 `payRoyaltyOnBehalf` 함수를 사용하여 [Royalty Module](/concepts/royalty-module/overview).

IP Asset에 [MockERC20](https://aeneid.storyscan.io/address/0xF2104833d386a2734a4eB3B8ad6FC6812F29E38E)로 지불하게 됩니다. 일반적으로는 \$WIP로 지불하지만, 테스트를 위해 토큰을 발행해야 하므로 MockERC20을 사용할 것입니다.

다음 시나리오를 이해하기 쉽도록, 부모 IP Asset이 자식 IP Asset과 50% `commercialRevShare`를 협상했다고 가정해보겠습니다.
...
이것은 10 \$MERC20을 `childIpId`의 [IP Royalty Vault](/concepts/royalty-module/ip-royalty-vault)로 전송합니다. 여기서 자식이 수익을 청구할 수 있습니다. 다음 섹션에서 이것이 작동하는 방식을 보게 될 것입니다.
...
IP Account ([IP Asset](/concepts/ip-asset/overview)을 나타내는 스마트 컨트랙트)는 처음 등록될 때 Royalty Token의 100%를 보유합니다. 따라서 일반적으로 대부분의 Royalty Token을 보유하고 있습니다.

작동 방식을 확인하고 결과를 검증하기 위해 `test/5_Royalty.t.sol` 아래에 테스트 파일을 만들어보겠습니다:
