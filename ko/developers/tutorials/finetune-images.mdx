...
description: FLUX 파인튜닝 API를 사용하여 이미지를 파인튜닝하고
  TypeScript로 Story에 출력물을 등록하는 방법을 배워보세요.
---
...
이 튜토리얼에서는 FLUX 파인튜닝 API를 사용하여 Story의 마스코트 "Ippy"의 여러 이미지를 가져와서 AI 모델을 파인튜닝하여 프롬프트와 함께 비슷한 이미지를 생성합니다. 그런 다음 Story에서 출력물을 수익화하고 IP를 보호할 것입니다.
...
생성형 텍스트-투-이미지 모델은 종종 창작자의 고유한 비전을 완전히 포착하지 못하고, 특정 객체, 브랜드 또는 시각적 스타일에 대한 지식이 불충분합니다. FLUX Pro 파인튜닝 API를 사용하면 창작자가 기존 이미지를 사용하여 AI를 파인튜닝하여 프롬프트와 함께 유사한 이미지를 생성할 수 있습니다.

이미지가 생성되면 IP를 성장시키고, 수익화하고, 보호하기 위해 Story에 IP로 등록할 것입니다.
...
3. 다음으로 이동: [Pinata](https://pinata.cloud/) 에서 새로운 API 키를 생성하세요. JWT를 다음에 추가하세요 `.env` file:

```yaml .env
PINATA_JWT=
```

4. 다음으로 이동: [BFL](https://api.us1.bfl.ai/auth/profile) 에서 새로운 API 키를 생성하세요. 새 키를 다음에 추가하세요 `.env` file:
...
5. 선호하는 Story RPC URL을 다음에 추가하세요 `.env` 파일. 우리가 제공하는 기본 공개 URL을 사용하시면 됩니다:
...
1. 프로젝트에 다음 이름의 폴더를 만드세요 `images`. 해당 폴더에 파인튜닝 학습에 사용할 이미지들을 추가하세요. *지원되는 형식: JPG, JPEG, PNG, WebP. 5개 이상의 이미지를 사용하는 것을 권장합니다.*
...
입력 이미지와 유사한 스타일의 이미지를 생성하려면 **finetune**을 생성해야 합니다. 파인튜닝은 입력 이미지를 모두 알고 있는 AI로, 새로운 이미지를 생성할 수 있습니다.

FLUX의 `/v1/finetune` API 경로를 호출하는 함수를 만들어봅시다. `flux` 폴더를 만들고, 그 안에 다음 이름의 파일을 추가하세요 `requestFinetuning.ts` 그리고 다음 코드를 추가하세요:
...
<Warning>
  **Warning: This is expensive!**
  ...
  새로운 파인튜닝을 생성하는 것은 비용이 많이 듭니다. 이 튜토리얼을 작성하는 시점에서 $2-$6 정도입니다. "FLUX PRO FINETUNE: TRAINING" 섹션을 다음에서 확인해주세요 [pricing page](https://docs.bfl.ml/pricing/).
</Warning>
...
파인튜닝된 모델로 이미지를 생성하기 전에 FLUX의 학습이 완료될 때까지 기다려야 합니다!
...
다음으로, 파일을 생성하고 `finetune-progress.ts` 그리고 방금 만든 `finetuneProgress` 함수를 호출하세요:
...
<Warning>
  **Warning: This costs money.**
  ...
  매우 저렴하지만 추론을 실행하는 데는 비용이 들며, 이 튜토리얼을 작성하는 시점에서 \$0.06-0.07 정도입니다. [가격 페이지](https://docs.bfl.ml/pricing/)의 "FLUX PRO FINETUNE: INFERENCE" 섹션을 검토해주세요.
</Warning>

이제 파인튜닝을 학습했으니 모델을 사용하여 이미지를 생성할 것입니다. "추론 실행"은 단순히 우리의 새로운 모델(해당 `finetune_id`로 식별됨)을 사용하여 새로운 이미지를 생성하는 것을 의미합니다.
...
<Note>
  **공식 문서**
  ...
  페이로드의 각 매개변수가 무엇인지 알아보려면 공식 `/v1/flux-pro-1.1-ultra-finetuned` API 문서를 [여기](https://api.us1.bfl.ai/scalar#tag/tasks/POST/v1/flux-pro-1.1-ultra-finetuned)에서 확인하세요.
</Note>
...
다음으로, 파일을 생성하고 `inference.ts` 그리고 방금 만든 `finetuneInference` 함수를 호출하세요. 첫 번째 매개변수는 위 스크립트를 실행해서 얻은 `finetune_id`이어야 하며, 두 번째 매개변수는 새 이미지를 생성하기 위한 프롬프트입니다.
...
우리의 `flux` 폴더에서 다음 이름의 파일을 생성하세요 `getInference.ts` 그리고 다음 코드를 추가하세요:
...
다음으로 이 이미지를 Story에 [IP Asset](/concepts/ip-asset)으로 등록하여 IP를 수익화하고 라이선스를 부여할 것입니다. `story` 폴더를 생성하고 `utils.ts` 파일을 추가하세요. 여기에 Story Config를 설정하기 위한 다음 코드를 추가하세요:
...
이제 새로운 추론을 만들었으니, 이미지 `sample` 파일을 IPFS에 직접 저장해야 합니다. 샘플은 임시적이기 때문입니다.
