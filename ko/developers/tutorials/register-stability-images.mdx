---
title: Stability AI 생성 이미지 등록 및 수익화
...
3. 다음으로 이동하세요 [Pinata 대시보드](https://app.pinata.cloud/developers/api-keys) 에서 새로운 API 키와 게이트웨이를 생성하세요. JWT와 게이트웨이를 다음에 추가하세요 `.env` file:
...
4. 다음으로 이동하세요 [Stability](https://platform.stability.ai/account/keys) 에서 새로운 API 키를 생성하세요. 새로운 키를 다음에 추가하세요 `.env` file:
...
이제 이미지가 있으니 접근할 수 있는 URL을 얻기 위해 IPFS에 저장해야 합니다. 이 튜토리얼에서는 [Pinata](https://pinata.cloud/)를 사용할 것입니다. 이는 이미지 저장을 쉽게 만들어주는 분산 저장 솔루션입니다.
...
이제 이미지를 생성하고 저장했으니 Story에서 이미지를 IP로 등록할 수 있습니다. 먼저 설정을 구성해 보겠습니다. `utils.ts` 파일에 다음 코드를 추가하세요:
...
다음을 보세요 [IPA 메타데이터 표준](/concepts/ip-asset/ipa-metadata-standard) 그리고 IP에 대한 메타데이터를 구성하세요. 아래와 같이 메타데이터를 적절히 포맷하세요:
...
누군가가 우리의 이미지를 사용할 때마다(상품, 광고 등에) 10 \$WIP의 초기 발행 수수료를 지불해야 하고, 파생 작품으로 수익을 얻을 때마다 5%의 로열티를 지불해야 하는 방식으로 이미지를 수익화하고 싶다고 가정해 보겠습니다.
...
축하합니다! 이제 귀하의 이미지가 상업적 라이선스 약관과 함께 Story에 등록되었습니다.
