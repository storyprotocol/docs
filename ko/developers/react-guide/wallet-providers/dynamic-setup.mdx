...
<Note>
  **Optional: Official Dynamic Docs**
  ...
  공식 Wagmi + Dynamic 설치 문서를[여기서](https://docs.dynamic.xyz/react-sdk/using-wagmi)확인하세요.
</Note>
...
1. 반드시`NEXT_PUBLIC_RPC_PROVIDER_URL`이 당신의`.env`파일에 설정되어 있어야 합니다.
   * 기본 공개 RPC(`https://aeneid.storyrpc.io`)나 다른 RPC를[여기서](/network/network-info/aeneid#rpcs)사용할 수 있습니다.
2. 반드시`NEXT_PUBLIC_DYNAMIC_ENV_ID`이 당신의`.env`파일에 설정되어 있어야 합니다.[Dynamic](https://app.dynamic.xyz/)에 로그인하고 프로젝트를 생성하세요.
