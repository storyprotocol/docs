---
title: Pyth
description: Pyth Network는 스마트 계약을 위한 가격 데이터와 검증 가능한 랜덤 함수(VRF)를 제공하는 분산형 오라클입니다. 기관 제공자로부터 직접 데이터를 소싱함으로써, Pyth는 투명성과 효율성을 유지하면서 안전하고 지연 시간이 낮은 가격 업데이트를 보장합니다.
---
