---
title: Redstone
description: Redstone은 스마트 컨트랙트와 DeFi 프로토콜을 위한 맞춤형 데이터 피드를 제공하도록 설계된 분산형 오라클 네트워크입니다. 데이터가 오프체인에서 서명되고 번들링된 후 필요할 때만 온체인으로 제출되는 독특한 오프체인 데이터 전달 모델을 사용합니다.
---

# 가격 피드

Redstone은 70개 이상의 블록체인에서 스마트 컨트랙트에 실시간 금융 데이터를 제공하며, 암호화폐, RWA, LRT, BTCFi 및 기타 새로운 자산을 다룹니다. 기관 및 암호화폐 네이티브 데이터를 결합하여, Redstone은 이상 감지, 시장 깊이 분석, 크로스 소스 분산 검사를 포함한 다층적 검증을 통해 신뢰성을 보장합니다.

Redstone의 [문서](https://docs.redstone.finance/docs/introduction) 가이드를 참조하여 애플리케이션을 가격 피드와 통합하세요.
