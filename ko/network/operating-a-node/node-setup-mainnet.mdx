...
The `story` 및 `geth` 바이너리는 Story 노드 실행에 필요한 클라이언트를 구성하며, 최신 `release` pages:
...
자신의 노드 설정을 재정의하려면 다음을 수행할 수 있습니다:
...
아래에 샘플을 나열합니다 `Systemd` Linux에서 사용할 수 있는 설정
...
실행 중인 상태를 확인하고 싶다면`story` 내부 JSONRPC/HTTP 엔드포인트를 쿼리하는 것이 도움이 됩니다. 다음은 실행할 수 있는 몇 가지 유용한 명령어입니다:
