---
title: 풀 노드
description: Story 메인넷을 위한 노드 설정 가이드
---

이 섹션에서는 Story 메인넷을 위한 노드 설정 방법을 안내합니다. Story는 실행과 합의 클라이언트를 분리하는 ETH PoS에서 영감을 받았습니다. 실행 클라이언트 `story-geth`는 EVM 블록을 `story` 합의 클라이언트로 Engine API를 통해 전달하며, ABCI++ 어댑터를 사용하여 EVM 상태를 CometBFT와 호환되게 만듭니다. 이러한 아키텍처로 인해 합의 효율성이 더 이상 실행 트랜잭션 처리량에 의해 제한되지 않습니다.

Story 노드 실행에 필요한 `story`와 `geth` 바이너리는 최신 릴리즈에서 사용할 수 있습니다 `release` pages:

* **`story-geth`실행 클라이언트:**
  * 릴리즈 링크: [**여기를 클릭하세요**](https://github.com/piplabs/story-geth/releases)
  * 최신 안정 버전 (v1.0.2): [**여기를 클릭**](https://github.com/piplabs/story-geth/releases/tag/v1.0.2)
* **`story`합의 클라이언트:**
  * 릴리즈 링크: [**여기를 클릭**](https://github.com/piplabs/story/releases)
  * 최신 안정 버전 (v1.1.1): [**여기를 클릭**](https://github.com/piplabs/story/releases/tag/v1.1.1)
...
*노드 기능에 필요한 모든 포트가 아래 설명된 대로 필요한지 확인하십시오*

* `story-geth`
  * 8545
    * HTTP를 통한 JSON-RPC API 인터페이스가 필요한 경우 필수
  * 8546
    * 웹소켓 상호작용에 필요
  * 30303 (TCP + API)
...
* `story`
  ...
  * 26657
    * Tendermint RPC 인터페이스가 필요한 경우 필수
  * 26660
    * 프로메테우스 메트릭스를 노출하려면 필요
...
자신만의 노드 설정을 재정의하려면 다음과 같이 할 수 있습니다:
...
* `${STORY_DATA_ROOT}/priv_validator_key.json` 는 검증자 키가 포함된 민감한 파일이지만, 자신의 것으로 대체할 수 있습니다

#### 사용자 정의 자동화

아래에 샘플을 나열합니다 `Systemd` Linux에서 사용할 수 있는 구성
...
실행 중인 상태를 확인하고 싶다면`story` 내부 JSONRPC/HTTP 엔드포인트를 쿼리하는 것이 도움이 됩니다. 다음은 실행하면 도움이 되는 몇 가지 명령어입니다:

* `curl localhost:26657/net_info | jq '.result.peers[].node_info.moniker'`
  * 이것은 노드가 모니커별로 동기화된 합의 피어 목록을 제공합니다
* `curl localhost:26657/health`
  * 이것은 노드가 정상인지 알려줍니다 - `{}` 는 정상임을 나타냅니다
